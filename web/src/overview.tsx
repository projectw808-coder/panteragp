  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total equity" icon="◈"
          value={perf.data ? usd(perf.data.equity) : a ? usd(a.equity) : '—'}
          note="Everything held, valued in dollars" />
        <StatCard label="Open P&L" icon="↗"
          value={a ? signed(openPnl) : '—'}
          tone={openPnl < 0 ? 'down' : openPnl > 0 ? 'up' : undefined}
          note={`${held.length} position${held.length === 1 ? '' : 's'} open`} />
        <StatCard label="Available" icon="≡"
          value={a ? usd(Number(a.balance)) : '—'}
          note={a ? `Cash in ${a.currency}` : ''} />
        <StatCard label="Margin used" icon="⊞"
          value={a ? usd(margin) : '—'}
...
function StatCard({ label, icon, value, note, tone }: {
  label: string; icon: string; value: string; note?: string; tone?: 'up' | 'down';
}) {
  return (
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img"
        ...
        <path d={path.area} fill="url(#earnings)" />
        <path d={path.line} fill="none" stroke="var(--color-ember)" strokeWidth="2"
